...
- Мониторинг запускается в момент добавления первого предмета.
- Изменить частоту запросов можно в файле bot.py (monitor = SkinMonitor(cache, update_interval=1800)), где интервал указан в секундах.
- Важно, при изменении интервала, учитывать то, что у steam api есть ограничение в 100-200 запросов в минуту, чтобы не заблочить себе IP.
...
Для кореектного запуска бота используйте VPN на локальной машине либо используйте выделенный сервер/прокси. Для личного использования мной был выбран вариант с выделенным сервером в стране без данной блокировки, чтобы не нагружать домашний пк круглосуточно.
